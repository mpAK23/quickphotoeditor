...
(function () {
  ...
  const chkCropPortrait = document.getElementById('chkCropPortrait');
  ...
  function initImageWorkspace(img) {
    ...
    chkCropPortrait.checked = img.naturalHeight > img.naturalWidth;
    exitCropMode();
    updateMainDisplay();
    autoBalanceNotans();
  }
  ...
  // Orientation pairs mapping for aspect ratios
  ...
  function isRatioPortrait(ratioVal) {
    if (!ratioVal || ratioVal === 'free' || ratioVal === '1:1') return false;
    const [rw, rh] = ratioVal.split(':').map(Number);
    return rh > rw;
  }

  function rotateCropRectDimensions() {
    if (!workingCanvas.width || !workingCanvas.height) return;

    const centerX = cropRect.x + cropRect.w / 2;
    const centerY = cropRect.y + cropRect.h / 2;

    // Convert normalized crop box to pixel dimensions
    const curPixW = cropRect.w * workingCanvas.width;
    const curPixH = cropRect.h * workingCanvas.height;

    // Swap pixel dimensions (90° rotation of crop tool without rotating photo)
    let newPixW = curPixH;
    let newPixH = curPixW;

    // Bound within 95% of stage dimensions
    const maxPixW = workingCanvas.width * 0.95;
    const maxPixH = workingCanvas.height * 0.95;

    if (newPixW > maxPixW) {
      const s = maxPixW / newPixW;
      newPixW *= s;
      newPixH *= s;
    }
    if (newPixH > maxPixH) {
      const s = maxPixH / newPixH;
      newPixW *= s;
      newPixH *= s;
    }

    const newW = newPixW / workingCanvas.width;
    const newH = newPixH / workingCanvas.height;
  ...
  function rotateCropTool() {
    if (!workingCanvas.width) return;
    if (!isCropMode) {
      enterCropMode();
    }

    const currentRatio = cropRatioSelect.value;
    if (currentRatio === 'free') {
      rotateCropRectDimensions();
      chkCropPortrait.checked = (cropRect.h * workingCanvas.height) > (cropRect.w * workingCanvas.width);
    } else if (currentRatio === '1:1') {
      adjustCropBoxForRatio();
    } else {
      if (PORTRAIT_PAIRS[currentRatio]) {
        cropRatioSelect.value = PORTRAIT_PAIRS[currentRatio];
      }
      chkCropPortrait.checked = isRatioPortrait(cropRatioSelect.value);
      rotateCropRectDimensions();
    }
  }

  // Rotate Crop Tool Button
  btnRotateCrop.addEventListener('click', () => {
    rotateCropTool();
  });

  // Portrait / Landscape Tick Checkbox
  chkCropPortrait.addEventListener('change', () => {
    if (!workingCanvas.width) return;
    if (!isCropMode) {
      enterCropMode();
    }

    const wantPortrait = chkCropPortrait.checked;
    const currentRatio = cropRatioSelect.value;

    if (currentRatio === 'free') {
      const isCurrentlyPortrait = (cropRect.h * workingCanvas.height) > (cropRect.w * workingCanvas.width);
      if (wantPortrait !== isCurrentlyPortrait) {
        rotateCropRectDimensions();
      }
    } else if (currentRatio === '1:1') {
      adjustCropBoxForRatio();
    } else {
      const isCurrentlyPortrait = isRatioPortrait(currentRatio);
      if (wantPortrait !== isCurrentlyPortrait && PORTRAIT_PAIRS[currentRatio]) {
        cropRatioSelect.value = PORTRAIT_PAIRS[currentRatio];
        rotateCropRectDimensions();
      }
    }
  ...
  // Keyboard shortcut 'X' to rotate crop tool
  ...
  cropRatioSelect.addEventListener('change', () => {
    ...
    if (val !== 'free' && val !== '1:1') {
      chkCropPortrait.checked = isRatioPortrait(val);
    }
    adjustCropBoxForRatio();
  });
  ...
  function handleDragMove(clientX, clientY) {
    ...
    if (activeHandle === 'move') {
      ...
      if (cropRatioSelect.value !== 'free' && activeHandle.length === 2) {
        ...
        const anchorRight = initialCropRect.x + initialCropRect.w;
        const anchorBottom = initialCropRect.y + initialCropRect.h;
